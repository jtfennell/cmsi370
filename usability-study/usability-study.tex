...
% JD: You mean *interface* knowledge---domain knowledge is the
%     real-world lens, such as simply the concept of a list of
%     songs, or a radio station, or playing music repeatedly.
The highest time 
...
% JD: What principle does this exemplify?  Or does it follow
...
% JD: This would only hold true when the window is fullscreen,
%     wouldn't it?
...
% JD: LaTeX considers ">" to be a math symbol, so they need to be
%     delimited by the math notation indicators \(   \).
it takes only 2
...
Spotify.
% JD: And in this case, it makes no difference whether the window
%     is maximized or not.
...
several functions.
% JD: Yes, although this confusion is not Fitts's Law.  Fitts is
%     exclusively a matter of size and distance.  Which poses the
%     question---if the iTunes buttons were merely bigger, with a
%     bigger symbol, would it have performed better?
...
% JD: Use \label and \ref to automatically insert the correct
%     figure numbers---look 'em up.
...
Navigation. % JD: Wait---iTunes or Spotify?
...
% JD: You called out a nice set of principles here...would have been
%     good to cite them.
